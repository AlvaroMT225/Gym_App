...
import { clientAchievements } from "@/lib/trainer-data"
...
export async function GET(
  ...
  const scopeResult = requireConsentScope(consentResult.consent, "achievements:read")
  if ("error" in scopeResult) return scopeResult.error

  return NextResponse.json({ achievements: clientAchievements[clientId] ?? [] })
}
