...
function parseGlobalRankingRow(value: unknown): GlobalRankingRow | null {
  ...
  return {
    ...
    globalScore: readNumber(value.global_score),
    scoreUpper: readNumber(value.score_upper),
    scoreLower: readNumber(value.score_lower),
    diversityFactor: readNumber(value.diversity_factor),
  };
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: rankings, error } = await adminSupabase
      ...
      .select("athlete_id, rank_position, previous_rank, global_score, score_upper, score_lower, diversity_factor")
      .eq("gym_id", gym_id)
    ...
    const parsedRankings = (Array.isArray(rankings) ? rankings : [])
      .map((row) => parseGlobalRankingRow(row))
    ...
    const formattedRankings = parsedRankings.map((entry) => {
      ...
      return {
        ...
        global_score: entry.globalScore,
        score_upper: entry.scoreUpper,
        score_lower: entry.scoreLower,
        diversity_factor: entry.diversityFactor,
    ...
    console.error("Error in /api/client/rankings/global:", error);
