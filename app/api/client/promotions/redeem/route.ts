...
function mapRedeemError(error: RpcErrorLike | null) {
  ...
  switch (code) {
    ...
    case "PROMO_IDENTIFIER_REQUIRED":
      return { error: "Codigo invalido", status: 400 }
...
async function fetchRedeemedPromotion(
  ...
  if (promotionError) {
    logRedeemFailure("POST /api/client/promotions/redeem promotion fetch error:", promotionError)
    throw new Error("PROMOTION_FETCH_FAILED")
  }

  if (redemptionError) {
    logRedeemFailure("POST /api/client/promotions/redeem redemption fetch error:", redemptionError)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      ...
      logRedeemFailure("POST /api/client/promotions/redeem rpc error:", error)
    ...
    if (!redeemed) {
      console.error("POST /api/client/promotions/redeem invalid rpc result:", data)
      return NextResponse.json(
        {
          error: "Error al canjear promocion",
    ...
    if (!promotion) {
      console.error("POST /api/client/promotions/redeem redeemed promotion not found:", {
